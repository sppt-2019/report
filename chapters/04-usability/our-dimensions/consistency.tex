...
\fs has two primary collection types: lists and arrays. The array-collection type can provides some benefits when looking up elements by index.  However, when looking up an element by index, dot notation is used to call the \ttt{[]} function, thus a lookup becomes \fsinline{array1.[0]}. This clashes with expectations of a C-family programmer where \csinline{array1[0]} is the norm.
