...
Several participants struggled with lambda expressions in \fs. Some of these participants expressed that they where not familiar with lambda expressions in \cs either, however even the participants with lambda expression experience from \cs had issues with them in \fs. The difference in syntax can be see in \lstref{lam-exp-syn}, with \cs on the left and \fs on the right.
...
\end{minted}
\end{minipage}
\caption{Lambda Expression Syntax}
