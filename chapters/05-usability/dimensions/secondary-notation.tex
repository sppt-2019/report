...
\begin{enumerate}
    ...
    \item F\# uses the arrow function signatures (e.g. \fsinline{val map : mapping : ('T -> 'U) -> list:'T list -> 'U list}, which is the signature of \ttt{List.map}) in its documentation. This notation indicates that a function of multiple parameters can be curried. Curring is not supported in C\# and is thus a concept that the programmers were unfamiliar with.
\end{enumerate}