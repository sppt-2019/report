...
\begin{table}[H]
	...
	\begin{tabular}{| l | c |}\hline
		...
		Investment Risk & 6  \rowEnd
		Investment Cost & 20 \rowEnd
		Investment Payoff & 13 \rowEnd
		Imperative Alternative & 3 \rowEnd
	\end{tabular}}
	\caption{Attention Investment Findings}
	\label{tab:att-inv-findings}
\end{table}

As can be seen in \tableref{att-inv-findings} the participants noted a high cost with a moderate payoff. Another point is that very few participants noted the problems with existing solutions, even when compared to \fsh. The high cost is attributed to loss of productivity while a developer learns to use \fsh.
